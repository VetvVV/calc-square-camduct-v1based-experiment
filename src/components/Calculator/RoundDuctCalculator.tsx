...
export function RoundDuctCalculator() {
  ...
  const result = useMemo(
    () => calculateRoundDuct({ A, B, quantity, material, thickness, C1, C2, internalJointType }),
      ...
      <div className="rounded-xl border border-[#d6cdb7] bg-[#f6f0df] shadow-sm">
        <div className="border-b border-[#d6cdb7] bg-[#efe6cf] px-4 py-3">
          <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div>
              <h3 className="text-lg font-semibold text-[#5b4e2a]">R-001 / {t('product.roundDuctStraight')}</h3>
              <p className="text-sm text-[#6d6247]">{t('product.roundDuctStraightDescription')}</p>
            </div>
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <ParameterField label={t('common.quantity')}>
              <input type="number" value={quantity} onChange={(e) => setQuantity(Number(e.target.value || 1))} className="h-9 w-full rounded-md border border-[#c9bea0] bg-white px-3" />
            </ParameterField>
            ...
            <ParameterField label={t('connector.c1')}>
              <select value={C1} onChange={(e) => setC1(e.target.value as 'none' | 'flange' | 'bandage')} className="h-9 w-full rounded-md border border-[#c9bea0] bg-white px-3">
            ...
            <ParameterField label={t('connector.c2')}>
              <select value={C2} onChange={(e) => setC2(e.target.value as 'none' | 'flange' | 'bandage')} className="h-9 w-full rounded-md border border-[#c9bea0] bg-white px-3">
                <option value="none">{t('connector.none')}</option>
                <option value="flange">{t('connector.flange')}</option>
                <option value="bandage">{t('connector.bandage')}</option>
              </select>
            ...
            <ParameterField label={t('common.internalJointType')}>
              <select value={internalJointType} onChange={(e) => setInternalJointType(e.target.value as 'none' | 'nipple' | 'coupling' | 'overlap' | 'zig_tractor')} className="h-9 w-full rounded-md border border-[#c9bea0] bg-white px-3">
